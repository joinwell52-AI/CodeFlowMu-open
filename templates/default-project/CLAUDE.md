...
- **MCP 文案**：热路径 / 生命周期收尾相关工具 docstring 同步 3.2.5 语义
  （必改 9 + 按需检查 5；见 `fcop-protocol.mdc` §MCP 工具 docstring 修改范围）。
...
### MCP 工具 docstring 修改范围（3.2.5 / Tool docstring scope）

3.2.5 **不要求**修改全部 MCP 工具，只修改会影响**执行热路径**和**生命周期
收尾语义**的工具说明（便利层 docstring，非协议语法变更）。

**必改 / Must update (9):**

| 工具 | 热路径 / 收尾语义 |
|---|---|
| `claim_task` | v3 开工前认领（inbox → active） |
| `read_task` | 执行前必读（与 `inspect_task` 配对） |
| `inspect_task` | 执行前 grammar 校验 |
| `fcop_check` | 轻量 drift 审计；**非**每次 report 后自动深度巡检 |
| `fcop_audit` | 深度协议体检；**≠** 业务验收 / archive |
| `write_report` | REPORT 停步（0.a.6）；**不**自动 finish/archive |
| `finish_task` | 生命周期目录移动；**≠** 业务完成 |
| `archive_task` | 治理归档（0.a.5）；**≠** 执行者默认 step 4 |
| `archive_to_history` | 深归档冷存储；须先 authorised `archive_task` |

**必须检查并按需修正 / Check & fix if needed (5):**

`fcop_report` · `write_task` · `submit_task` · `approve_task` · `reject_task`

**不纳入 3.2.5 修改范围 / Out of scope** unless a tool docstring currently
claims: mandatory claim/read/inspect before every action, auto
finish/archive after `write_report`, auto deep audit via `fcop_check`, or
"archive = business done".
