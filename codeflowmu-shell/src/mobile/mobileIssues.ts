...
export function slimMobileIssue(row: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    root_cause_status: row.root_cause_status ?? "unknown",
    closure_record: row.closure_record,
    closure_digest: row.closure_digest,
    promotion_status: row.promotion_status ?? "not_promoted",
    severity: row.severity,
    blocking_level: row.blocking_level ?? row.blocking ?? row.severity,
    source_task: row.source_task ?? row.task_id,
    source_report: row.source_report,
    owner: row.owner,
    recipient: row.recipient,
    reason: row.reason,
    root_fault_id: row.root_fault_id,
